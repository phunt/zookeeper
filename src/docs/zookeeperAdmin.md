...
1. Set the Java heap size. This is very important to avoid
  ...
  machine.
1. Install the ZooKeeper Server Package. It can be downloaded
  from:
  [http://zookeeper.apache.org/releases.html](http://zookeeper.apache.org/releases.html)
1. Create a configuration file. This file can be called anything.
...
1. Create an initialization marker file *initialize*
  ...
  ensemble.
1. If your configuration file is set up, you can start a
  ZooKeeper server:
  **
...
1. Test your deployment by connecting to the hosts:
  In Java, you can run the following command to execute
  simple operations:
  **
      $ bin/zkCli.sh -server 127.0.0.1:2181**
{anchor:singleAndDevSetup}
...
the [Bindings](zookeeperProgrammers.html#Bindings)
...
### Provisioning {#sc_provisioning}
{anchor:strengthsAndLimitations}

### Things to Consider: ZooKeeper Strengths and Limitations {#sc_strengthsAndLimitations}
{anchor:administering}
...
policy that administrators can use. The [API docs](ext:api/index) contains details on
...
**
    ...
    OutOfMemoryError** occurs.  This is achieved
